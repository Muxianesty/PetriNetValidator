...
if __name__ == "__main__":
    ...
    original, converted = apply_fpp_rule(net, convertible_p)
    visualizeNet(original, dir_path + "1-1.png")
    visualizeNet(converted, dir_path + "1-2.png")
    original, converted = apply_fpt_rule(net, convertible_t)
    visualizeNet(original, dir_path + "2-1.png")
    visualizeNet(converted, dir_path + "2-2.png")
    target_tpp = [place for place in interface.net.places if place.name == "p4"][0]
    convertible_tpp = [transition for transition in net.net.transitions if transition.name == "t6"][0]
    original, converted = apply_lte_rule(net, convertible_tpp)
