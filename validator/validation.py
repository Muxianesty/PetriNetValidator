            ...
            if mode > 0 and len(int_dict[net_key]) < len(net_dict[net_key]):
                ...
                if int_plcs_count > net_plcs_count and isLocalLabel(net_key):
                    for place in m_net.net.places:
                        in_labels = set(arc.source.label for arc in place.in_arcs)
                        out_labels = set(arc.target.label for arc in place.out_arcs)
                        in_net_src = 0
                        in_net_dst = 0
                        for inner_place in m_net.net.places:
                            p_in_labels = set(arc.source.label for arc in inner_place.in_arcs)
                            p_out_labels = set(arc.target.label for arc in inner_place.out_arcs)
                        ...
                        in_int_src = 0
                        in_int_dst = 0
                        for inner_place in interface.net.places:
                            p_in_labels = set(arc.source.label for arc in inner_place.in_arcs)
                            p_out_labels = set(arc.target.label for arc in inner_place.out_arcs)
                            if in_labels == p_in_labels and {EMPTY_LABEL} == p_out_labels:
                                in_int_src += 1
